...
def evaluate(submission, truth, vtype='SNV', ignorechroms=None, truthmask=True):
    ...
    print '{0} records found in {1}'.format(len(trulist), truvcfh.filename)

    ''' count records in truth vcf, track contigs/chromosomes '''
    for k, trurec in enumerate(trulist):
        if relevant(trurec, vtype, ignorechroms):
            trurecs += 1
            truchroms[trurec.CHROM] = True
    print "{0} out of {1} are of type {2}, that's {3:.2%}".format(
        trurecs, k + 1, vtype, float(trurecs) / (k + 1))
    ...
    for k, subrec in enumerate(subvcfh):
        ...
        if k > 0 and k % 5e3 == 5e3 - 1:
            print '{0} records processed in {1}'.format(k + 1, subvcfh.filename)
    print '{0} records processed in {1}'.format(k + 1, subvcfh.filename)
